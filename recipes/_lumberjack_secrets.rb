...
# Recipe:: _lumberjack_secrets
#
# Copyright 2015, Autumn Wang
#

lumberjack_data_bag = node['logstash']['instance_default']['lumberjack_data_bag']

unless lumberjack_data_bag
  Chef::Log.warn("node['logstash']['instance_default']['lumberjack_data_bag'] was set to false. Not configuring lumberjack or secrets.")
  return
end
...
if !lumberjack_secrets.nil? && lumberjack_secrets['key'] && lumberjack_secrets['certificate']
  node.run_state['lumberjack_decoded_key'] = lumberjack_secrets['key']
  node.run_state['lumberjack_decoded_certificate'] = lumberjack_secrets['certificate']
